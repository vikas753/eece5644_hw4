...
def svmvalidate(svm,data,labels,filename):
    ...
    if(filename == ""):
        plot_scatter_sh(data,labels,"")
        show_plot_svm_contour(data,svm,"",labels)
    return loss
...
def svmFullTraining(data,labels):
    ...
    for C in [1,10,100,200,500,700,1000]:
        for Gamma in [0.00001,0.0001,0.001,0.01,0.1,1,10]:
            ...
            loss=svmvalidate(model,data,labels,"abcd")
            #print(" C = " , C, " Gamma = " , Gamma , "loss =" , loss )
            if(loss < min_loss):
                min_loss = loss
                best_model = model
                C_opt = C
...
def KfoldCrossValidation(data,labels,kfold):
    ...
    for train_index, test_index in lpo_cross_val_model.split(data_np):
        ...
        print(" Validation Index = " , validationIndex , " loss_dataset = " , loss_dataset)
...
with open('dataXhw42100test.csv', newline='') as f:
    reader = csv.reader(f)
    filedataX_toy = [tuple(row) for row in reader]

with open('dataYhw42100test.csv', newline='') as f:
    reader = csv.reader(f)
    filedataY_toy = [tuple(row) for row in reader]
    
with open('labels100.csv', newline='') as f:
    reader = csv.reader(f)
    labels_toy = [tuple(row) for row in reader]
...
with open('dataXhw4210000test.csv', newline='') as f:
    reader = csv.reader(f)
    filedataX_test = [tuple(row) for row in reader]

with open('dataYhw4210000test.csv', newline='') as f:
    reader = csv.reader(f)
...
filedataX_toy = [float(i) for i in filedataX_toy[0]]    
filedataY_toy = [float(j) for j in filedataY_toy[0]]
labels_toy = [int(k)   for k in labels_toy[0]]

filedataX_toy = np.array(filedataX_toy)
filedataY_toy = np.array(filedataY_toy)
labels_toy = np.array(labels_toy)
...
for i in range(filedataX_toy.size):
    Data_toy.append([filedataX_toy[i],filedataY_toy[i]])
